...
<div class="collapse" id="collapseExample" style="margin-bottom:20px;">
    <div class="filterbody">
        ...
        <div class="row">
            <div class="row">
                @if (isset($filterData))
                @foreach($filterData['filters'] as $filter => $filtername)
                    @if($filtername === 'selectgroup')
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="label">{{ $options['label'] }}</label>
                                <select class="formcontrol2" id="{{ $filters }}" onchange="applyFilters('{{ $filter }}')">
                                    @foreach ($data[$filter] as $groupLabel => $options)
                    ...
                    @elseif ($options['inputType'] === 'select')
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="label">{{ $options['label'] }}</label>
                                <select class="formcontrol2" id="{{ $filter }}Filter" onchange="applyFilters('{{ $filter }}')">
                                    <option value="All">All</option>
                                    @foreach ($data[$filter] as $option)
                                    <option value="{{ $option }}">{{ $option }}</option>
                                    @endforeach
                                </select>
                            </div>
